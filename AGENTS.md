...
You should write apps using TypeScript, not JavaScript.
You should use verbose commentary in the code. Your comments should be such that an intermediate level TypeScript developers with limited Forge experience to understand.
...
You must install packages using the project's package manager (this project uses pnpm) after creating the app and every time you add or update a dependency.
...
If the user closes the tunnel after making changes, you MUST ask them whether they would like to redeploy their app so that their recent changes are deployed.
