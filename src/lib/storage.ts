...
class PageStorage {
    ...
    async getPage(issueId: string | number, pageId: string | number) {
        ...
        return value
            ? PageAttachmentLinkSchema.parse(value)
            : undefined;
    }
    ...
    async getPages(issueId: string | number) {
        ...
        const validatedResults = z.array(
            z.object({
                key: z.string(),
                value: z.string()
            }))
        ...
        const pages = validatedResults.map(json => PageAttachmentLinkSchema.parse(JSON.parse(json.value)))
        return pages;
    }

    private getKey(...args: (string | number)[]): string {
        const parsedArgs = z.array(stringOrNumber).safeParse(args);
        if (!parsedArgs.success) throw new InvalidStorageKeyError("Invalid storage key");
        for (const arg of args) {
            if (!arg) throw new InvalidStorageKeyError("Arg cannot be empty");
            if (typeof arg === "string" && arg.includes("-")) throw new InvalidStorageKeyError("Arg cannot contain -");
        }
